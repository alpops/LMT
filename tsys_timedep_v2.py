...
from numpy import histogram,ones,exp,savez,where
...
def fit_ref_voltage(cal_data, nbins=300):

    vals,x  = histogram(cal_data, nbins)
    x = (x[1:]+x[:-1])/2.


    mean = (max(cal_data)+min(cal_data))/2.0
    ref_guess = cal_data[where(cal_data>mean)].mean()
        cold_guess = cal_data[where(cal_data<mean)].mean()

        #print ref_guess,cold_guess

        expected = (ref_guess, 1., 50., cold_guess, 1., 50.)
